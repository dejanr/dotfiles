...
import { closeSync, constants, cpSync, openSync, statSync, utimesSync, writeSync } from "node:fs";
import {
	access,
	appendFile,
	mkdir,
...
const MANAGED_BY = "pi-extension-ds4";
...
const EXTENSION_DIR = dirname(EXTENSION_FILE);
...
let bundledRuntimeDir: string | undefined;
...
function selectedModelQuant(): ModelQuant {
	...
	throw new Error(`DeepSeek V4 Flash requires at least 128 GB RAM for q2; detected ${ramGb.toFixed(1)} GB`);
}
...
function resolveBundledRuntimeDir(): string {
	if (bundledRuntimeDir) return bundledRuntimeDir;

	const candidates = [
		join(EXTENSION_DIR, "vendor", "ds4"),
		join(EXTENSION_DIR, "..", "vendor", "ds4"),
	...
	for (const candidate of candidates) {
		try {
			if (
				statSync(join(candidate, "download_model.sh")).isFile() &&
				statSync(join(candidate, "Makefile")).isFile() &&
				statSync(join(candidate, "ds4_server.c")).isFile()
			) {
	...
	throw new Error(`Cannot find bundled ds4 runtime relative to ${EXTENSION_FILE}`);
}

async function resolvePackagedServerBinary(): Promise<string | undefined> {
	if (resolvedServerBinary) return resolvedServerBinary;

	const whichBinary = (await execCapture("which", ["ds4-server"], 2_000))?.trim();
	const candidates = [
		process.env.DS4_SERVER_BINARY,
		resolve(join(EXTENSION_DIR, "..", "..", "ds4-runtime", "bin", "ds4-server")),
		whichBinary,
	...
	for (const candidate of candidates) {
		try {
			...
			resolvedServerBinary = candidate;
			return candidate;
		} catch {}
	}
...
async function syncDirectory(sourceDir: string, targetDir: string): Promise<void> {
	...
	for (const entry of entries) {
		...
		if (entry.isSymbolicLink()) {
			cpSync(sourcePath, targetPath, { dereference: false, force: true, recursive: false });
			continue;
		}
...
async function ensureBundledRuntime(onStatus?: StatusCallback): Promise<string> {
	if (process.platform !== "darwin") {
		throw new Error("ds4 extension currently supports darwin/Metal only");
	}

	const bundledDir = resolveBundledRuntimeDir();
	if (!(await isDs4Checkout(bundledDir))) {
		throw new Error(`Bundled ds4 runtime at ${bundledDir} is incomplete`);
	}

	onStatus?.("syncing bundled ds4 runtime");
	await syncDirectory(bundledDir, SUPPORT_DIR);
	return SUPPORT_DIR;
}
...
async function resolveWatchdogScript(runtimeDir: string): Promise<string> {
	...
	try {
		...
		throw new Error(`Cannot find ${WATCHDOG_SCRIPT_NAME} in ds4 runtime ${runtimeDir}`);
	}
}
...
async function resolveRuntimeDirLocked(onStatus?: StatusCallback): Promise<string> {
	...
	resolvedRuntimeDir = await ensureBundledRuntime(onStatus);
	try {
		resolvedRuntimeDir = await realpath(resolvedRuntimeDir);
	} catch {}
	return resolvedRuntimeDir;
}

async function ensureBuilt(_runtimeDir: string, onStatus?: StatusCallback): Promise<void> {
	const packagedBinary = await resolvePackagedServerBinary();
	if (!packagedBinary) {
		throw new Error("Cannot find packaged ds4-server. Install the darwin ds4 runtime package or set DS4_SERVER_BINARY.");
	}
	resolvedServerBinary = packagedBinary;
	onStatus?.("using packaged ds4-server");
}
...
async function startServerLocked(runtimeDir: string): Promise<void> {
	const binary = resolvedServerBinary ?? (await resolvePackagedServerBinary());
	if (!binary) {
		throw new Error("Cannot find packaged ds4-server. Install the darwin ds4 runtime package or set DS4_SERVER_BINARY.");
	}
			...
			try {
				await ctx.ui.custom<void>(
					(tui, theme, _keybindings, done) => {
						viewer = new Ds4LogViewer(tui as LogTui, theme as LogTheme, done);
						return viewer;
					},
					{
						overlay: true,
...
export default function ds4Extension(pi: ExtensionAPI) {
	...
	pi.on("before_provider_request", async (_event, ctx: ExtensionContext) => {
